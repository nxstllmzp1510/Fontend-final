  ...
  return (
    <div className="max-w-[1200px] w-full">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between mb-7">
        ...
        <Link to="/admin/inventory-logs" className="w-fit rounded-md bg-[#5a403e] px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-[#4a322f]">ดูทุกล็อตสินค้า</Link>
      </div>
      ...
      <section className="rounded-xl border border-[#e8e3dd] bg-white shadow-sm overflow-hidden mb-6">
...
function Metric({ label, value, detail, tone }: { label: string; value: number; detail: string; tone: 'normal' | 'warning' | 'danger' }) {
  ...
  return <div className={`rounded-xl border p-5 shadow-sm ${toneClass}`}><p className="text-xs font-bold uppercase tracking-wide text-[#7B726B]">{label}</p><p className="my-1 text-3xl font-bold text-[#302221]">{value} <span className="text-sm font-medium">ล็อต</span></p><p className="text-xs text-[#7B726B]">{detail}</p></div>;
}
